...
Смотрите файлы ```testXXX.cpp``` из каталога [test/](test/) в качестве примеров использования библиотеки logengine в дополнение к [примерам использования](#примеры-использования) ниже.
Смотрите файл [example.lfg](test/testFiles/example.lfg) для полного описания параметров конфигурационного файла LogEngine.

Вопросы и сообщения об ошибках приветствуются. 
Пожалуйста, используйте систему отслеживания ошибок Github по адресу https://github.com/lasersquad0/LogEngine2/issues.
...
* Пользовательский логгер с обратным вызовом
* Обьекты логирования:
  * Обычные файлы журналов
  * Ротируемые файлы журналов
  * Логирование в консоль (stdio и stderr)
  * Символьная строка
* Настраиваемые шаблоны для строк журнала, которые могут включать информацию:
  * Дата и/или время
...
```
---
#### Создание логгеров типов: stdout/stderr, файловый логгер
```c++
